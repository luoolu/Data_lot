...
# -*- coding: utf-8 -*-
"""
【高级强化学习彩票预测系统 – 综合优化版 v19】

修订内容：
1. 引入 Cosine Beta 调度策略用于 Diffusion 模型（可选线性或余弦调度）。
2. 调整命中奖励权重（bonus_multiplier 从 4.0 提升至 5.0）。
3. 在奖励函数、集成策略等模块中做细节调整，以期提升预测命中数量。
4. 增加最近一个月每天命中数量图的绘制与保存。
5. 进一步优化部分模拟过程中的噪声设定，降低噪声幅度，提高预测稳定性。
6. 增加 Diffusion 模型预测结果的单独保存。
7. **修正命中图的绘制范围，仅绘制到预测日的前一天。**
"""
...
def simulate_episode_with_prob(env, model, future_date, num_runs=10):
    ...
    for i in range(num_runs):
        ...
        while env.current_step < env.max_steps:
            ...
            env.step(action)
        # 降低噪声强度，提高预测稳定性
        noise = np.random.normal(0, 0.01, size=prob_accum.shape)
        prob_accum += noise
...
def simulate_diffusion_with_prob(env, diff_model, target_date, num_runs=10):
    ...
    for i in range(num_runs):
        ...
        # 降低噪声强度，减少随机性影响
        prob_accum += np.random.normal(0, 0.01, size=prob_accum.shape)
...
def main():
    ...
    # 注意：epochs 参数可根据实际情况调节，进一步优化可考虑早停策略
    ...
    def save_prediction(pred_list, model_name):
        ...
        csv_path = os.path.join(dst_dir, f"{model_name}_prediction_v19_FIXED_TOTALTIMESTEPS_{TOTAL_TIMESTEPS}.csv")
    ...
    final_md_path = os.path.join(dst_dir, f"RL_lottery_predictor_v19_FIXED_{TOTAL_TIMESTEPS}.md")
    ...
    plot_save_path = os.path.join(dst_dir, "daily_hits_RL_lottery_predictor_v19_.png")
