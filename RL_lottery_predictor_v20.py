...
# -*- coding: utf-8 -*-
"""
【高级强化学习彩票预测系统 – 综合优化版 v20】

修订内容：
1. 将 bonus_multiplier 从 5.0 提升至 6.0，并调整奖励函数中动态权重更新步长（由 0.001 调整为 0.0005），以期提升命中奖励并使权重调整更平滑。
2. 模拟过程中降低噪声幅度（从标准差 0.01 调整为 0.005），以进一步提升预测稳定性。
3. 扩散模型训练中增加早停机制，并将默认训练 epoch 数降低（默认 epochs=500），提高效率并防止过拟合。
4. 集成预测结果中温度缩放参数调整为 0.65，以获得更精确的概率校准。
5. 集成权重调优试验次数由 30 增加至 50 次，以便获得更佳的最优权重。

其他逻辑基本沿用 V19 代码结构与算法。
...
class LotterySequentialEnv(gym.Env):
    def __init__(self, data_df, extra_feature_cols, extra_max, extra_mean, extra_std, reward_weights=None,
                 bonus_multiplier=6.0):  # 修改：bonus_multiplier 调整为 6.0
    ...
    def update_reward_weights(self):
        # 修改：更新步长由 0.001 调整为 0.0005，更新更加平滑
...
class DiffusionModel:
    ...
    def train_model(self, env, known_draws, epochs=500, batch_size=128, early_stopping_patience=20, min_delta=1e-4):
        # 新增早停机制以防止过拟合
        X_context = []
        Y_target = []
...
def main():
    ...
    # -------------------------- 环境与模型初始化 -----------------------------
    env_qrdqn = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=6.0)
    env_ppo = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=6.0)
    env_a2c = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=6.0)
    ...
    # -------------------------- 模型评估 -----------------------------
    env_eval = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=6.0)
    ...
    # 修改：epochs 由 100000 降低为 500，并增加早停机制
    ...
    # 仅绘制从 (df["Date"].max() - 30 天) 到 (future_date 的前一天) 的日期命中
    eval_dates = df[
    ...
    # -------------------------- 预测未来一天 -----------------------------
    env_pred = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=6.0)
    ...
    def save_prediction(pred_list, model_name):
        ...
        csv_path = os.path.join(dst_dir, f"{model_name}_prediction_v20_FIXED_TOTALTIMESTEPS_{TOTAL_TIMESTEPS}.csv")
    ...
    final_md_path = os.path.join(dst_dir, f"RL_lottery_predictor_v20_FIXED_{TOTAL_TIMESTEPS}.md")
    ...
    plot_save_path = os.path.join(dst_dir, "daily_hits_RL_lottery_predictor_v20.png")
