...
def train_model(rank, world_size, dataset_path):
    ...
    train_loader = DataLoader(dataset, batch_size=1320, sampler=train_sampler)

    device = torch.device(f"cuda:{rank}")
    d_model = 1024
    model = TransformerModel(d_model, 64, 32).to(device)
    # 计算并打印模型参数总量
    ...
    if rank == 0:
        torch.save(model.state_dict(),
                   "/media/luolu/3cc826d6-a968-4df2-9ae7-ee2c324675bd/home/xkjs/Downloads/model_dir/dlt_best_model.pth")
        plt.plot(losses)
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
...
def main():
    ...
    dataset_path = "/home/luolu/PycharmProjects/lottery/Data/dlt2_asc20230418.csv"
