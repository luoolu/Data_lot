...
"""
强化学习彩票预测系统优化版本：
1. 丰富特征工程：新增外部数据（如天气、搜索热度）与彩票延迟统计特征
2. 模型结构：采用门控 Transformer 特征提取器（GatedTransformerFeatureExtractor），提高长程依赖建模能力
3. 训练策略与集成方法：保留 QRDQN、PPO、A2C 多模型训练，增加基于不确定性加权的概率输出估计
4. 评估指标：新增预期校准误差（ECE）和预测多样性指标
数据路径和保存路径保持原有设置
"""
...
def add_external_features(df):
    # 尝试加载外部数据（例如天气、搜索热度等），路径请自行准备文件 external_data.csv
    external_path = "/home/luolu/PycharmProjects/NeuralForecast/Utils/GetData/external_data.csv"
    try:
        ext_df = pd.read_csv(external_path, encoding='utf-8', parse_dates=["Date"])
        logging.info("成功加载外部数据")
        # 假定外部数据中包含列：'Date', 'temperature', 'humidity', 'search_trend'
        df = pd.merge(df, ext_df, on="Date", how="left")
        ext_cols = ["temperature", "humidity", "search_trend"]
        for col in ext_cols:
...
def evaluate_model_advanced(env, models, df, eval_dates, num_runs=5):
    ...
    return avg_hits, ece_value, diversity_score
...
def plot_hit_trend(df, models, env, extra_feature_cols, extra_max, dst_dir, total_timesteps):
    ...
    plt.legend()
    plt.tight_layout()
    hit_trend_path = os.path.join(dst_dir, f"RL_lottery_predictor_v3_TOTAL_TIMESTEPS_{total_timesteps}.png")
...
def main():
    ...
    # 若外部数据存在，则添加其特征列，例如 "temperature", "humidity", "search_trend"
    for col in ["temperature", "humidity", "search_trend"]:
        if col in df.columns:
    ...
    # 使用高级评估函数对最近30天（或其它）进行综合评估，获取平均命中、ECE和多样性指标
    recent_dates = df[df['Date'] >= df['Date'].max() - pd.Timedelta(days=30)]['Date'].tolist()
    avg_hit, ece_value, diversity_score = evaluate_model_advanced(env_eval, [model_qrdqn, model_ppo, model_a2c],
    ...
    def save_prediction(pred, model_name):
        ...
        csv_path = os.path.join(dst_dir, f"{model_name}_v3_prediction_TOTAL_TIMESTEPS_{TOTAL_TIMESTEPS}.csv")
    ...
    # 模型集成：动态加权融合三个模型的预测结果（同时也可以考虑基于贝叶斯不确定性）
    ...
    pred_csv_path = os.path.join(dst_dir, f"RL_lottery_predictor_v3_TOTAL_TIMESTEPS_{TOTAL_TIMESTEPS}.md")
