...
"""
强化学习彩票预测系统优化版本：
1. 丰富特征工程：新增外部数据（如天气、搜索热度、风速）与彩票延迟统计特征及日期相关特征
...
class LotterySequentialEnv(gym.Env):
    ...
    def _compute_static_features(self):
        ...
        # 此外，外部数据中的温度、湿度、风速已经通过 add_external_features 合并到数据中
...
def plot_hit_trend(df, models, env, extra_feature_cols, extra_max, dst_dir, total_timesteps):
    ...
    plt.legend()
    plt.tight_layout()
    hit_trend_path = os.path.join(dst_dir, f"RL_lottery_predictor_v4_TOTAL_TIMESTEPS_{total_timesteps}.png")
...
def main():
    ...
    # 使用高级评估函数对最近30天进行综合评估，获取平均命中、ECE和多样性指标
    ...
    def save_prediction(pred, model_name):
        ...
        csv_path = os.path.join(dst_dir, f"{model_name}_v4_prediction_TOTAL_TIMESTEPS_{TOTAL_TIMESTEPS}.csv")
    ...
    # 模型集成：动态加权融合三个模型的预测结果（可进一步考虑贝叶斯不确定性加权）
    ...
    pred_csv_path = os.path.join(dst_dir, f"RL_lottery_predictor_v4_TOTAL_TIMESTEPS_{TOTAL_TIMESTEPS}.md")
