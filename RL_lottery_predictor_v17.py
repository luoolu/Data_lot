...
# -*- coding: utf-8 -*-
"""
【高级强化学习彩票预测系统 – 综合优化版 v17 修复版】

改进要点：
1. 基于现有强化学习架构，集成扩散模型（Diffusion Model）用于提升预测准确性；
2. 采用元学习策略进行模型集成（Stacking/可学习权重组合），融合强化学习模型与扩散模型的预测结果；
3. 引入 Optuna 自动超参数调优机制，实现端到端的性能优化；
4. 新增图网络（GNN）优化：将构建的彩球共现图融入特征提取器中，采用简单的图卷积操作提升信息传递效果；
5. 修复“Expected all tensors to be on the same device”的设备不匹配错误，使用 register_buffer 来托管 adjacency。
"""
...
def build_lottery_graph(df):
    ...
    row_sum[row_sum == 0] = 1
    A_norm = A / row_sum
    return th.tensor(A_norm)  # 归一化后的邻接矩阵, 初始在CPU, 后面会在FeatureExtractor中register_buffer
...
class GatedTransformerGNNFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, seq_len=10, d_model=128, nhead=8,
        ...
        # 关键修复点：将 adjacency 注册为 buffer，这样在GPU/CPU切换时，adjacency 会跟随模型一起移动，避免设备不匹配
        if adjacency is not None:
            self.register_buffer("adjacency", adjacency)
        else:
            self.adjacency = None

        # 根据 observation_space 计算静态特征维度：
        obs_dim = observation_space.shape[0]
        # 末尾 80 是 chosen_mask，接着 1 是 progress，1 是 avg_reward，共 82
    ...
    def forward(self, observations: th.Tensor) -> th.Tensor:
        ...
        # 将静态特征作为长度 1 的序列输入
        seq = static_feats.unsqueeze(1)  # (B, 1, static_dim)
        seq = self.proj(seq)             # (B, 1, d_model)
        seq = seq.transpose(0, 1)        # (1, B, d_model)

        seq = self.pos_encoder(seq)      # (1, B, d_model)
        transformer_out = self.transformer_encoder(seq)  # (1, B, d_model)
        transformer_out = transformer_out.squeeze(0)     # (B, d_model)

        # 如果提供了邻接矩阵，则进行图卷积操作
        if self.adjacency is not None:
            # 此时 adjacency 已注册为 buffer，位于与模型相同的 device
            gnn_input = th.matmul(chosen_mask, self.adjacency)  # (B, 80)
        else:
            gnn_input = chosen_mask

        gnn_out = F.relu(self.gnn_layer(gnn_input))  # (B, gnn_hidden_dim)

        combined = th.cat([transformer_out, gnn_out], dim=1)  # (B, d_model + gnn_hidden_dim)
...
def train_model(algorithm, env, policy_kwargs, total_timesteps, update_timesteps, model_save_path):
    ...
    if algorithm == "QRDQN":
        ...
        # 这里我们并没有真正使用 QRDQN 的 distributional 功能，仅将其当作 DQN；如果你想用稳定基线的 QR-DQN，需要对应的 fork 或扩展
        model = DQN(
...
class DiffusionModel:
    def __init__(self, static_feat_dim, T=50, time_embed_dim=32, hidden_dim=256):
        self.T = T
...
def main():
    ...
    # 在 policy_kwargs 中传入图结构邻接矩阵，使特征提取器能够利用图网络信息
    policy_kwargs = {
        ...
        "features_extractor_kwargs": {
            ...
            "temperature": 0.7,
            "learnable_pe": True,
            "adjacency": lottery_graph  # 传入图网络邻接矩阵(在GatedTransformerGNNFeatureExtractor里register_buffer)
        }
    }

    env_qrdqn = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=4.0)
    env_ppo = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=4.0)
    env_a2c = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=4.0)
    ...
    env_eval = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=4.0)
    ...
    diff_model = DiffusionModel(static_feat_dim=static_feat_dim, T=50, time_embed_dim=32, hidden_dim=256)
    ...
    env_pred = LotterySequentialEnv(df, extra_feature_cols, extra_max, extra_mean, extra_std, bonus_multiplier=4.0)
    ...
    def save_prediction(pred_list, model_name):
        ...
        csv_path = os.path.join(dst_dir, f"{model_name}_prediction_v17_FIXED_TOTALTIMESTEPS_{TOTAL_TIMESTEPS}.csv")
    ...
    final_md_path = os.path.join(dst_dir, f"RL_lottery_predictor_v17_FIXED_{TOTAL_TIMESTEPS}.md")
