...
"""
高级强化学习彩票预测系统：
1. 特征工程优化：增加遗漏值特征（每个号码距上次出现的间隔期数），同时保留原有周期性、跨期统计、上下文增强特征。
2. 模型结构：基于门控 Transformer 提取全局特征，再融入图网络模块（使用历史共现邻接矩阵和可学习嵌入）实现号码间注意力消息传递，
   用于生成每个号码的预测得分；未来可扩展强化学习与注意力新融合方案。
3. 预测评估指标：在命中数、ECE、Brier 分数、预测多样性指标之外，增加号码覆盖率、预测分布熵（集中度）和 Top-N 指标。
数据读取路径和结果保存路径保持原有设置。
"""
...
# 新增：遗漏值特征，统计每个号码距上次开出间隔期数，并将 80 个遗漏值作为新特征加入数据（列名 omission_1 ... omission_80）
...
def build_lottery_graph(df):
    ...
    A_norm = A / row_sum
    return th.tensor(A_norm)  # 返回 Torch tensor


# =============================================================================
# 2. Transformer-GNN特征提取器 —— 融合门控Transformer与图网络模块
# =============================================================================
...
class GatedTransformerEncoderLayer(nn.Module):
    """
    门控Transformer编码层：在标准自注意力模块后增加门控残差连接，
    同时在前馈部分增加门控融合。
    """
...
class GatedTransformerGNNFeatureExtractor(BaseFeaturesExtractor):
    """
    该模块先利用门控Transformer提取全局特征，再将全局特征与每个号码的可学习嵌入融合，
    结合预先构建的号码共现图，利用简单图卷积实现号码间的信息消息传递，
    最后直接输出每个号码的得分（作为动作 logits），输出维度固定为80。
    """
    def __init__(self, observation_space, seq_len=10, d_model=128, nhead=8,
                 num_layers=2, transformer_out_dim=128, gnn_hidden_dim=64,
                 d_emb=32, dropout=0.1):
        # 输出维度为80，对应 80 个可选号码（动作）
    ...
    def forward(self, observations):
        ...
        # 扩展：将全局特征复制到每个号码节点
        global_expanded = global_feature.unsqueeze(1).expand(B, 80, -1)  # (B, 80, transformer_out_dim)
        # 彩票号码嵌入： (80, d_emb) -> (B, 80, d_emb)
        lottery_embed = self.lottery_embedding.unsqueeze(0).expand(B, -1, -1)
        # 融合全局特征与号码嵌入
        fused = th.cat([global_expanded, lottery_embed], dim=-1)  # (B, 80, transformer_out_dim + d_emb)
        # 利用邻接矩阵进行简单图消息传递：对每个节点，求其邻接节点的加权和
        # 邻接矩阵 shape: (80, 80), expand至 (B,80,80)
        A = self.adjacency.unsqueeze(0).expand(B, -1, -1)
        aggregated = th.bmm(A, fused)  # (B, 80, transformer_out_dim + d_emb)
        # 将经过图卷积后的每个节点映射到一个标量分数
        node_logits = self.gnn_fc(aggregated).squeeze(-1)  # (B, 80)
        return node_logits


# =============================================================================
# 3. 强化学习环境 —— 初始化时确定 observation_space
# =============================================================================
...
# =============================================================================
# 4. 模型预测与评估辅助函数（增加覆盖率、集中度、Top-N 指标）
# =============================================================================
...
# 新增：计算预测分布的熵作为集中度指标，熵越低说明预测越集中
def compute_entropy(probs):
    # 避免 log0 加一个极小值
    p = np.clip(probs, 1e-6, 1.0)
    return -np.sum(p * np.log(p)) / np.log(len(p))  # 归一化到[0,1]

# 新增：对一组评估日期计算多项指标，包括命中、ECE、Brier、多样性、号码覆盖率、平均熵以及 Top-N 精度（例如 Top-10）
def evaluate_model_advanced(env, models, df, eval_dates, num_runs=5, top_n=10):
    hit_counts = []
    ensemble_predictions = []
    weights = []
    # 记录所有评估日期的预测概率（用于后续计算覆盖率、熵、Top-N指标）
    all_probs = []
    union_predicted = set()
    topn_precisions = []
    for model in models:
    ...
    coverage = len(union_predicted) / 80.0  # 号码覆盖率
    # 计算平均熵（归一化熵，1表示均匀分布；0表示完全集中）
...
def plot_hit_trend(df, models, env, extra_feature_cols, extra_max, dst_dir, total_timesteps):
    ...
    plt.legend()
    plt.tight_layout()
    hit_trend_path = os.path.join(dst_dir, f"RL_lottery_predictor_v6_TOTAL_TIMESTEPS_{total_timesteps}.png")
...
def train_model(algorithm, env, policy_kwargs, total_timesteps, update_timesteps, model_save_path):
    if algorithm == "QRDQN":
        ...
        model.learn(total_timesteps=total_timesteps)
    # 未来可扩展更多先进算法，如 Decision Transformer、Diffusion Policy 等
...
def main():
    ...
    df = add_omission_features(df)  # 新增遗漏值特征
    # 更新特征列表，添加新生成的特征
    extra_feature_cols = extra_feature_cols + [
        ...
        # 此外也可加入遗漏值统计特征，可考虑对 80 个遗漏值计算统计量（例如均值、最大、最小）——本示例仅保留原始遗漏值
    ]
    ...
    # 使用融合了图网络模块的门控Transformer-GNN特征提取器
    policy_kwargs = {
    ...
    # 使用高级评估函数：返回平均命中、ECE、多样性、Brier、号码覆盖率、平均熵、Top-{top_n} 精度
    ...
    def save_prediction(pred, model_name):
        ...
        csv_path = os.path.join(dst_dir, f"{model_name}_v6_prediction_TOTAL_TIMESTEPS_{TOTAL_TIMESTEPS}.csv")
    ...
    pred_csv_path = os.path.join(dst_dir, f"RL_lottery_predictor_v6_TOTAL_TIMESTEPS_{TOTAL_TIMESTEPS}.md")
    ...
    # 将预先构建的共现图传递给新特征提取器（假设各模型都采用相同的 policy_kwargs 设置）
    # 本示例中，后续更新模型参数需自行加载后赋值：例如，model.policy.features_extractor.adjacency = lottery_graph
    # 此处仅输出提示信息
    logging.info("请确保在模型加载后，将 lottery_graph 赋值到各模型的 features_extractor.adjacency 属性中。")
